    ...
    */
	...
	void LateUpdate()
	{
		if ( !GameManager.instance.pausado )
		{
			...
			if ( dragEnabled )
			{
				if ( !GameManager.instance.passaroInDragging )
				{
					panControl();
				}
			}
...
;
	...
	public void doTouchControls()
	{
		...
		if ( Input.touchCount == 2 )
		{
			...
			float prevTouchDeltaMag, touchDeltaMag, deltaMagnitudeDiff;
			...
			{
				Camera.main.orthographicSize += deltaMagnitudeDiff * ( touchDragSpeed * Time.deltaTime );
				Camera.main.orthographicSize = Mathf.Clamp( Camera.main.orthographicSize, 5f, 10f );
			}
		}
	}
	...
	public void panControl()
	{
		...
		// if mouse is down
		if ( Input.GetMouseButton( ( int )mouseButton ) && Input.touchCount == 1 )
		{
			click++;
			if ( click > 1 )
			{
